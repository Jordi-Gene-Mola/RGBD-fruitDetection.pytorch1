...
def parse_args():
  ...
  parser.add_argument('--save_dir', dest='save_dir',
                      help='directory to save models', default="/mnt/gpid08/users/jordi.gene/faster_rcnn/data/kinect_fruits_models",
...
if __name__ == '__main__':
  ...
  if args.dataset == "pascal_voc":
      ...
      #args.set_cfgs = ['ANCHOR_SCALES', '[2, 4, 8, 16]', 'ANCHOR_RATIOS', '[0.5,1,2]', 'MAX_NUM_GT_BOXES', '100']
      #args.set_cfgs = ['ANCHOR_SCALES', list(np.array(args.anchor).astype(np.int)), 'ANCHOR_RATIOS', '[0.5,1,2]',
      #                 'MAX_NUM_GT_BOXES', '100']
      args.set_cfgs = ['ANCHOR_SCALES', list(np.array(args.anchor).astype(np.int)), 'ANCHOR_RATIOS', list(np.array(args.anchor_ratio).astype(np.float)),
                       'MAX_NUM_GT_BOXES', '100']
  ...
  if not os.path.isdir('output_train_nou'):
      os.mkdir('output_train_nou')
