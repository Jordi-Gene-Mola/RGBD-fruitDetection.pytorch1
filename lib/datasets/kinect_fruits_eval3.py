...
def voc_eval3(image_ids,
    ...
    r = np.zeros((len(IoU), len(confid)))
    p = np.zeros((len(IoU), len(confid)))
    ap = np.zeros((len(IoU)))
    ...
    for ovthresh in IoU:
        ...
        for d in range(nd):
            class_recs[image_ids[d]]['det']=[False] * len(class_recs[image_ids[d]]['det'])
        ...
        if BB.shape[0] > 0:
            ...
            ap[IoUidx] = voc_ap(rec, prec, use_07_metric)
            print('min IoU = {:.2f}'.format(ovthresh))
            print('AP for {} = {:.4f}'.format(classname, ap[IoUidx]))

            for minconfid in confid:
                # confids=np.array(args.minconfid).astype(np.float)
                # confid = confids[2]
                confididx = confididx + 1
                r[IoUidx, confididx] = rec[sum(confidence > minconfid)]
                p[IoUidx, confididx] = prec[sum(confidence > minconfid)]
