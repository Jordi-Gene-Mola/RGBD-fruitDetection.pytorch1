...
from datasets.kinect_fruits_eval2 import voc_eval2
from datasets.kinect_fruits_eval3 import voc_eval3
...
if __name__ == '__main__':
  ...
  epoch = 0
  AP_epoch = np.zeros((len(files),len(args.ovthresh)))
  ...
  meanTime = np.zeros((len(files)))
  #print('Mean ap vector:\n')
  #print(AP_epoch)

  ##load anotations
  print('estem aqui 01')
  imagesetfile = os.path.join(imdb._devkit_path, 'apples', 'sets', imdb._image_set + '.txt')
  ...
  annopath = os.path.join(imdb._devkit_path, 'apples', 'square_annotations1','{:s}.xml')
  print('estem aqui 02')
  ...
  for file in files:
    ...
    for cls_ind, cls in enumerate(imdb.classes):
        ...
        for im_ind, index in enumerate(imdb.image_index):
            ...
            for k in xrange(dets.shape[0]):
                image_ids += [index]
                confidence = np.append(confidence, dets[k, -1])
                BB = np.concatenate((BB, [[dets[k, 0] + 1, dets[k, 1] + 1, dets[k, 2] + 1, dets[k, 3] + 1]]), axis=0)
        #
    ...
    for i, cls in enumerate(imdb._classes):
        ...
        #filename='/work/jgene/faster_rcnn/data/kinect_fruits_dataset/apples/results/s77_det_Poma.txt'
        ...
        #IoU=np.array([0.4,0.5,0.6,0.7]).astype(np.float)
        #confid=np.array([0.05,0.1,0.15,0.2,0.25,0.3,0.35,0.4,0.45,0.5,0.55,0.6,0.65,0.7,0.75,0.8,0.85,0.9,0.95]).astype(np.float)

        #detpath='/work/jgene/faster_rcnn/data/kinect_fruits_dataset/apples/results/s77_det_Poma.txt'
        #annopath='/work/jgene/faster_rcnn/data/kinect_fruits_dataset/apples/square_annotations1/{:s}.xml'
        #imagesetfile='/work/jgene/faster_rcnn/data/kinect_fruits_dataset/apples/sets/test.txt'
        #classname='Poma'
        #cachedir='/work/jgene/faster_rcnn/data/kinect_fruits_dataset/annotations_cache'
        ...
        rec, prec, ap, r, p = voc_eval3(image_ids, confidence, BB, imagenames, recs, cls, np.array(args.ovthresh).astype(np.float), np.array(args.minconfid).astype(np.float))
        #rec, prec, ap, r, p = voc_eval2(image_ids,confidence,BB, annopath, imagesetfile, cls, cachedir,np.array(args.ovthresh).astype(np.float),np.array(args.minconfid).astype(np.float))
        #rec, prec, ap, r, p = voc_eval(filename, annopath, imagesetfile, cls, cachedir,  np.array(args.ovthresh).astype(np.float), np.array(args.minconfid).astype(np.float))
        #rec, prec, ap, r, p = voc_eval(filename, annopath, imagesetfile, cls, cachedir,IoU,confid)

    meanTime[epoch] = np.mean(InferTime)
...
mat = np.matrix(meanTime)
with open('output/test_s' + str(args.checksession) + '/meanTime_s' + str(args.checksession) + '.txt', 'wb') as f:
    for line in mat:
        np.savetxt(f, line, fmt='%.4f')
